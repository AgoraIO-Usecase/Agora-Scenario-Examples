...
class LargeClassController: BaseViewController {
    ...
    public func eventHandler() {
        ...
        layoutView.onpublishMicrophoneTrackClosure = { [weak self]  isPublish in
            guard let self = self else { return }
            self.channelMediaOptions.publishMicrophoneTrack = isPublish
            self.channelMediaOptions.clientRoleType = isPublish ? .broadcaster : .audience
        ...
        layoutView.onPublishCameraTrackClosre = { [weak self]  isPublish in
            guard let self = self else { return }
            self.channelMediaOptions.publishCameraTrack = isPublish
            self.channelMediaOptions.clientRoleType = isPublish ? .broadcaster : .audience
            self.agoraKit?.updateChannel(with: self.channelMediaOptions)
        }
    ...
    private func joinChannel(channelName: String, uid: UInt) {
        channelMediaOptions.clientRoleType = getRole(uid: UserInfo.uid)
        channelMediaOptions.publishCameraTrack = getRole(uid: UserInfo.uid) == .broadcaster
        channelMediaOptions.publishMicrophoneTrack = getRole(uid: UserInfo.uid) == .broadcaster
