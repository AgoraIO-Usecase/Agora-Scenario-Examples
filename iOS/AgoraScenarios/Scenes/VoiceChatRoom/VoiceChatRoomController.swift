...
class VoiceChatRoomController: BaseViewController {
    ...
    private func muteAudioHandler(isVoice: Bool) {
        ...
        option.publishMicrophoneTrack = getRole(uid: UserInfo.uid) == .broadcaster ? true : isVoice
        option.publishCameraTrack = getRole(uid: UserInfo.uid) == .broadcaster
        if getRole(uid: UserInfo.uid) == .audience {
            option.clientRoleType = isVoice ? .broadcaster : .audience
        }
