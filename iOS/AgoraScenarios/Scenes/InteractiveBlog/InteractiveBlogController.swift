...
class InteractiveBlogController: BaseViewController {
    ...
    public func eventHandler() {
        ...
        liveView.enableAudioClosure = { [weak self] isEnable in
            guard let self = self else { return }
            self.channelMediaOptions.publishMicrophoneTrack = isEnable
            self.channelMediaOptions.clientRoleType = isEnable ? AgoraClientRole.broadcaster : AgoraClientRole.audience
