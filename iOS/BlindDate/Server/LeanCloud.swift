...
extension Room {
    ...
    static func querySpeakerCount(room: LCObject) throws -> Int {
        let query = LCQuery(className: Member.TABLE)
        try query.where(Member.ROOM, .equalTo(room))
        try query.where(Member.ROLE, .notEqualTo(RoomRole.listener.rawValue))
        return query.count().intValue
    }
...
extension Member {
    static func from(object: LCObject, room: Room) throws -> Member {
        ...
        let _role = object.get(ROLE)?.intValue ?? 0
        let role: RoomRole
        switch _role {
        ...
        return Member(id: id, isMuted: isMuted, isSelfMuted: isSelfMuted, role: role, room: room, streamId: streamId, user: user)
    }
    ...
    func toObject() throws -> LCObject {
        ...
        try object.set(Member.ROLE, value: self.role.rawValue)
    ...
    func asListener() -> Observable<Result<Void>> {
        return Database.save {
            ...
            let object = LCObject(className: Member.TABLE, objectId: self.id)
            try object.set(Member.ROLE, value: RoomRole.listener.rawValue)
            return object
        }
        .map { $0.transform() }
    }
