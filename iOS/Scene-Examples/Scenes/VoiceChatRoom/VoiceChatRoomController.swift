...
class VoiceChatRoomController: BaseViewController {
    ...
    private func joinChannel() {
        channelMediaOptions.publishLocalAudio = getRole(uid: UserInfo.uid) == .broadcaster
    ...
    private func muteAudioHandler(isVoice: Bool) {
        if getRole(uid: UserInfo.uid) == .audience {
            agoraKit?.setClientRole(isVoice ? .broadcaster : .audience)
        }
    ...
    private func eventHandler() {
        ...
        soundEffectView.onTapSwitchClosure = { [weak self] isOn in
            self?.agoraKit?.setAudioEffectPreset(isOn ? .pitchCorrection : .audioEffectOff)
        }
        ...
        mucisView.onTapPlayButtonClosure = { [weak self] mucisModel, isPlay in
            ...
            if isPlay {
                self?.agoraKit?.startAudioMixing(model.url, loopback: true, replace: true, cycle: 1, startPos: 0)
                return
            }
