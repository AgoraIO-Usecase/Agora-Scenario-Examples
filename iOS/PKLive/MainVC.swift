...
class MainVC: UIViewController {
    ...
    var connectItem: UIBarButtonItem!
    
    init(loginInfo: LoginInfo) {
        super.init(nibName: nil, bundle: nil)
        self.vm = MainVM(loginInfo: loginInfo)
    ...
    func setup() {
        ...
        connectItem = UIBarButtonItem(title: "connect", style: .plain, target: self, action: #selector(connectOnClick))
    ...
    @objc func connectOnClick() {
        ...
        let action2 = UIAlertAction(title: "ok", style: .default, handler: { [weak self](_) in
            ...
            self?.vm.makeConnect(roomName: text)
        })
        alertVC.addAction(action1)
        alertVC.addAction(action2)
        present(alertVC, animated: true, completion: nil)
...
extension MainVC: MainVMDelegate {
    ...
    func mainVMDidUpdateRenderInfos(renders: [RenderInfo]) {
        if loginInfo.role == .audience {
            navigationItem.rightBarButtonItems =  [closeItem, connectItem]
        }
        else {
