...
public class ChatManager {
    ...
    private void initRtcEngine(Context context) {
        ...
        try {
            ...
            TokenGenerator.gen(context, roomInfo.roomId, uid, token -> {
                ...
                publishAudio(isPublish);
                rtcEngine.joinChannel(token, roomInfo.roomId, "", uid);
            });
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private void publishAudio(boolean publish) {
        ...
        rtcEngine.setClientRole(publish ? Constants.CLIENT_ROLE_BROADCASTER : Constants.CLIENT_ROLE_AUDIENCE);
        rtcEngine.muteLocalAudioStream(!publish);
        rtcEngine.enableLocalAudio(publish);
    }
