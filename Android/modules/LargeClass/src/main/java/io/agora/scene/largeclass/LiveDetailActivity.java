...
public class LiveDetailActivity extends BaseActivity<LargeClassLiveDetailActivityBinding> {
    ...
    private void initRtcEngine() {
        try {
            ...
            options.clientRoleType = isLocalTeacher()? Constants.CLIENT_ROLE_BROADCASTER : Constants.CLIENT_ROLE_AUDIENCE;
    ...
    private void updateUserClientRole(RoomManager.UserInfo userInfo){
        ...
        runOnUiThread(() -> {
            ...
            ChannelMediaOptions options = new ChannelMediaOptions();
            options.clientRoleType = isAccept ? Constants.CLIENT_ROLE_BROADCASTER: Constants.CLIENT_ROLE_AUDIENCE;
            options.autoSubscribeAudio = options.autoSubscribeVideo = true;
            options.publishCameraTrack = isAccept && localUser.isEnableVideo;
            options.publishMicrophoneTrack = isAccept && localUser.isEnableAudio;
            ...
            public void onBindViewHolder(@NonNull BindingViewHolder<LargeClassLiveVideoItemBinding> holder, int position) {
                ...
                if (item.isEnableVideo) {
                    ...
                    if (!(holder.binding.videoContainer.getTag() instanceof Integer
                        ...
                        videoView = new TextureView(LiveDetailActivity.this);
