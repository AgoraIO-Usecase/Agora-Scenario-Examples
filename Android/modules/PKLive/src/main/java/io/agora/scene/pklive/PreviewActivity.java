...
public class PreviewActivity extends AppCompatActivity {
    ...
    private void showVideoSettingDialog() {
        ...
        for (VideoEncoderConfiguration.VideoDimensions sVideoDimension : io.agora.scene.pklive.Constants.sVideoDimensions) {
            resolutions.add(new Size(sVideoDimension.width, sVideoDimension.height));
        }
        List<Integer> frameRates = new ArrayList<>();
        for (VideoEncoderConfiguration.FRAME_RATE sFrameRate : io.agora.scene.pklive.Constants.sFrameRates) {
        ...
        new VideoSettingDialog(PreviewActivity.this)
                ...
                .setDefaultValues(new Size(io.agora.scene.pklive.Constants.encoderConfiguration.dimensions.width, io.agora.scene.pklive.Constants.encoderConfiguration.dimensions.height),
                        io.agora.scene.pklive.Constants.encoderConfiguration.frameRate, io.agora.scene.pklive.Constants.encoderConfiguration.bitrate)
                .setOnValuesChangeListener(new VideoSettingDialog.OnValuesChangeListener() {
                    @Override
                    public void onResolutionChanged(Size resolution) {
                        io.agora.scene.pklive.Constants.encoderConfiguration.dimensions = new VideoEncoderConfiguration.VideoDimensions(resolution.getWidth(), resolution.getHeight());
                    }

                    @Override
                    public void onFrameRateChanged(int framerate) {
                        io.agora.scene.pklive.Constants.encoderConfiguration.frameRate = framerate;
                    }

                    @Override
                    public void onBitrateChanged(int bitrate) {
                        io.agora.scene.pklive.Constants.encoderConfiguration.bitrate = bitrate;
                    }
                })
                .show();
    }
