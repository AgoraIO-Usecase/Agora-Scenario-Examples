...
public class LiveDetailActivity extends AppCompatActivity {
    ...
    private void joinChannel() {
        rtcEngine.setClientRole(
                localIsRoomOwner() ? io.agora.rtc.Constants.CLIENT_ROLE_BROADCASTER : io.agora.rtc.Constants.CLIENT_ROLE_AUDIENCE
        );
        channelMediaOptions = new ChannelMediaOptions();
        channelMediaOptions.publishLocalAudio = localIsRoomOwner();
        channelMediaOptions.publishLocalAudio = localIsRoomOwner();
        channelMediaOptions.autoSubscribeVideo = true;
        channelMediaOptions.autoSubscribeAudio = true;
        int uid = Integer.parseInt(RoomManager.getCacheUserId());
        TokenGenerator.gen(LiveDetailActivity.this, roomInfo.roomId, uid, ret -> rtcEngine.joinChannel(ret, roomInfo.roomId, "", uid, channelMediaOptions));
    }
    ...
    private void renderMainPreview() {
        ...
        if (localIsRoomOwner()) {
            ...
            rtcEngine.setupRemoteVideo(new VideoCanvas(videoView, io.agora.rtc.Constants.RENDER_MODE_HIDDEN, Integer.parseInt(roomInfo.userId)));
        }
    }
    ...
    private void showSettingDialog() {
        new LiveToolsDialog(LiveDetailActivity.this)
                ...
                .addToolItem(LiveToolsDialog.TOOL_ITEM_VIDEO, channelMediaOptions.publishLocalVideo, (view, item) -> {
                    channelMediaOptions.publishLocalVideo = item.activated;
                    rtcEngine.muteLocalVideoStream(item.activated);
                    rtcEngine.enableLocalVideo(item.activated);
                })
                .addToolItem(LiveToolsDialog.TOOL_ITEM_SPEAKER, channelMediaOptions.publishLocalAudio, (view, item) -> {
                    channelMediaOptions.publishLocalAudio = item.activated;
                    rtcEngine.muteLocalAudioStream(item.activated);
    ...
    private void upSeat(RoomManager.UserInfo userInfo) {
        ...
        if (userInfo.userId.equals(RoomManager.getCacheUserId())) {
            SurfaceView renderView = RtcEngine.CreateRendererView(this);
            ...
            rtcEngine.setupLocalVideo(new VideoCanvas(renderView,
                    io.agora.rtc.Constants.RENDER_MODE_HIDDEN, 0));
            rtcEngine.startPreview();
            ...
            rtcEngine.setupRemoteVideo(new VideoCanvas(renderView,
                    io.agora.rtc.Constants.RENDER_MODE_HIDDEN, Integer.parseInt(userInfo.userId)));
        }
    }
