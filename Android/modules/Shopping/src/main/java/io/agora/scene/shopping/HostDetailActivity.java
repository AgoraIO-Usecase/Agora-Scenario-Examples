...
public class HostDetailActivity extends AppCompatActivity {
    ...
    private final RoomManager.DataCallback<RoomManager.PKApplyInfoModel> pkApplyInfoModelDataCallback = data -> runOnUiThread(() -> {
        ...
        if (data.status == RoomManager.PKApplyInfoStatus.invite) {
            new AlertDialog.Builder(this)
                    .setMessage(R.string.shopping_pk_invite_tip)
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        mBinding.bottomView.setFun1Visible(true)
                ...
                .setFun1ClickListener(v -> {
                    new ProductListDialog(roomInfo.roomId, false, HostDetailActivity.this)
                            .show();
                });
    ...
    private void initRtcEngine() {
        try {
            ...
            rtcEngine.setCameraCapturerConfiguration(new CameraCapturerConfiguration(io.agora.scene.shopping.Constants.cameraDirection));
            rtcEngine.setVideoEncoderConfiguration(io.agora.scene.shopping.Constants.encoderConfiguration);
