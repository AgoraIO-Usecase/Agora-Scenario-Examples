...
public class DonateDialog extends BaseBottomSheetDialogFragment<DialogDonateBinding> implements OnItemClickListener<Gift> {
    public static final String TAG = "DonateDialog";

    private RoomViewModel roomViewModel;

    private BaseRecyclerViewAdapter<ItemDialogGiftBinding, Gift, GiftHolder> mAdapter;
    ...
    private void doDonate(View view) {
        if (mAdapter.selectedIndex < 0 || mAdapter.selectedIndex >= mAdapter.getItemCount()) {
            ...
            if (gift != null) {
                GiftInfo giftInfo = new GiftInfo(GiftUtil.getGiftResNameById(requireContext(), gift.getId()), gift.getValue(), gift.getName(), GameApplication.getInstance().user.getUserId());
